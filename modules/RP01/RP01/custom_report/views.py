"""Custom Report Designer — drag-and-drop pivot over the continuing vessel-call
dataset: historical rows from mis_vessel_master (RP01 vessel-master upload) plus
live rows built from VCN01 (call + parcels), LDUD01 (SOF timings) and LUEU01
(actual handled quantity). Both legs share one column set so pivots span the
historical/live boundary seamlessly.
"""
...
VALID_SOURCES = {'vessel-calls'}

# date_col key → (historical mis_vessel_master expr, live expr)
DATE_COL_FILTERS = {
    'nor':              ('nor', "COALESCE(l.nor_tendered, v.nor_tendered::TEXT, v.doc_date)"),
    'cargo_completion': ('cargo_completion', 'l.discharge_completed'),
}
DEFAULT_DATE_COL = 'nor'
...
def _date_where(expr, from_date, to_date):
    """Timestamp BETWEEN filter on a text date expression (reference pattern)."""
    from_val = (from_date or '').replace('T', ' ') or '1900-01-01 00:00'
    to_val = (to_date or '').replace('T', ' ') or '2999-12-31 23:59'
    clause = (f"NULLIF({expr}, '') IS NOT NULL"
              f" AND NULLIF({expr}, '')::TIMESTAMP BETWEEN %s AND %s")
...
def pivot_data(source):
    ...
    date_col = request.args.get('date_col', DEFAULT_DATE_COL)
    hist_expr, live_expr = DATE_COL_FILTERS.get(date_col, DATE_COL_FILTERS[DEFAULT_DATE_COL])

    hist_where, hist_params = _date_where(hist_expr, from_date, to_date)
    live_where, live_params = _date_where(live_expr, from_date, to_date)
